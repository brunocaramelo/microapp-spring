...
public class CategoryController {
    ...
    private final CategoryService localService;

    @GetMapping("/")
    public List<CategoryEntity> getAllUsers(@RequestParam(value = "page", required = false, defaultValue = "0") int page,
                                            @RequestParam(value = "pageSize", required = false, defaultValue = "50") int pageSize) {
        return localService.getAll(page, pageSize);
    }

    @GetMapping("/{id}")
    public ResponseEntity<CategoryEntity> getUserById(@PathVariable int id) {
